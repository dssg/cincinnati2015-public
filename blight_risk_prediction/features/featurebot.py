...
from blight_risk_prediction.features import ner, parcel, outcome, tax, crime, census

logger = logging.getLogger(__name__)

# for every feature-set to generate, you need to register a function that can generate a dataframe containing the
# new features. you also need to set the database table in which to store the feature
FeatureToGenerate=namedtuple("FeatureToGenerate", ["table", "generator_function"])
...
features_to_generate = [FeatureToGenerate("tax", tax.make_tax_features),
                        ...
                        FeatureToGenerate("named_entities",ner.make_owner_features),
                        FeatureToGenerate("house_type", parcel.make_house_type_features),
                        FeatureToGenerate("parc_area", parcel.make_size_of_prop),
                        FeatureToGenerate("parc_year", parcel.make_year_built),
                        FeatureToGenerate("census_2010", census.make_census_features)]
...
def generate_features():
    """
    Generate labels and features for all inspections in the inspections database.

    The features and labels will be stored in schema "features". This schema must exist in the database before calling
