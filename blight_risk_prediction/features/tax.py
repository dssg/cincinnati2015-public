...
def load_three_year_home_values(db_connection):
    taxes = """SELECT inspections.parcel_id, inspections.inspection_date, year AS taxyear,
               ...
               AND EXTRACT(YEAR FROM inspections.inspection_date) > taxes.year
               AND EXTRACT(YEAR FROM inspections.inspection_date) - 3 <= taxes.year;"""
...
def load_three_year_foreclosures(db_connection):
    foreclosures = """SELECT inspections.parcel_id, inspections.inspection_date, taxes.year,
                             taxes.forcl_flag AS foreclosure
                      FROM parcels_inspections AS inspections
...
def calculate_means(values):
    ...
    means = values.reset_index().groupby(["parcel_id", "inspection_date"]).mean()
...
def calculate_value_changes(group):
    ...
    first_year_values = group.loc[first_year][["mkt_total_val", "mkt_land_val", "mkt_impr_val"]]
    last_year_values = group.loc[last_year][["mkt_total_val", "mkt_land_val", "mkt_impr_val"]]
...
def calculate_relative_value_changes(group):
    ...
    return pd.Series({"rel_change_market_value": value_changes["mkt_total_val"],
...
def make_tax_features(db_connection):
    ...
    changes = values.reset_index().groupby(["parcel_id", "inspection_date"]).apply(calculate_value_changes)
    ...
    changes = values.reset_index().groupby(["parcel_id", "inspection_date"]).apply(calculate_relative_value_changes)
    ...
    foreclosures = foreclosures.reset_index().groupby(["parcel_id", "inspection_date"]).apply(count_foreclosed_years)
